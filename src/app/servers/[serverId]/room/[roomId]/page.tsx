...
const RoomPage = () => {
    ...
    useEffect(() => {
        const getMessages = async () => {
            try {
                const res = await axios.post<any>(`/api/servers/getmsg`, { room: roomId })
                setMessages(res.data.data);
            } catch (err) {
                console.log('Mesajlar yüklenirken hata oluştu')
            }
        }
    ...
    const sendMessage = async () => {
        ...
        try {
            const res = await axios.post<any>("/api/servers/addmsg", {
                user_id: user!.id,
                room_id: roomId,
                content: message,
                writed_name: user!.seen_name,
                writed_img: user!.profile_img
            })
        ...
        <div className="w-full h-[calc(100%-48px)] p-2 relative">
            ...
            <div className='w-full h-full flex flex-col gap-3 overflow-y-scroll p-2 custom-scrollbar'>
                {messages.length === 0 ? (
                    ...
                    messages.map((msg, i) => {
                        const isReceived = msg.writed_by !== user!.id;
                                    ...
                                    <div className='w-8 h-8 shrink-0 mb-1'>
                                        {showAvatar && (
                                            <div className='w-8 h-8 bg-linear-to-br from-gray-100 to-gray-200 rounded-full flex items-center justify-center overflow-hidden'>
                                                {msg?.writed_img ? (
                                                    <img src={msg.writed_img} alt="profil" className='w-full h-full object-cover' />
                                                ) : (
